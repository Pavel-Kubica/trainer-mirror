...
let isReferenceSolutionAccessible = false

onMounted(() => {
  loadIsReferenceSolutionAccessible()
})
...
const loadIsReferenceSolutionAccessible = async () => {
  try {
    isReferenceSolutionAccessible = await moduleApi.getReferenceSolutionAccessibleAnswer(moduleData.value.id);
  } catch (error) {
    isReferenceSolutionAccessible = false
  }
...
<template>
  <v-card v-if="!moduleData.teacher" flat class="d-flex flex-column">
    <ModuleBottomOverlay v-model="codeFile" accept=".c, .cpp, .cxx, .h, .hpp" multiple
                         ...
                         :reset-callback="() => codeData.codeModuleIdCheck = -1"
                         @update:modelValue="codeFileInserted">
      <template #prepend-actions>
        <v-menu location="left">
          ...
          <v-list>
            ...
            <v-list-item v-if="isReferenceSolutionAccessible" @click="() => downloadRef()">
