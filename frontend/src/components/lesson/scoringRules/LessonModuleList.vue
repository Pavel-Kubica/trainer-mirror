...
import LessonModuleListDialogRow from "@/components/lesson/scoringRules/LessonModuleListDialogRow.vue";
...
const customSearch = (lessonModule) => (!searchId.value || `${lessonModule.module.id}`.includes(searchId.value))
    && (!searchText.value || lessonModule.module.name.toLowerCase().includes(searchText.value.toLowerCase()))
      ...
      <template #content>
        <div class="d-flex mt-4" style="gap: 16px">
          ...
          <v-select v-model="searchAuthors[0]" :label="t('$vuetify.module_list_header_author')"
                    :items="[...new Set(lessonModules.map((md) => md.module.author))].sort()" />
        ...
        <v-table v-if="lessonModules.length" class="mb-4">
          ...
          <tbody>
            <LessonModuleListDialogRow :custom-search="customSearch" :dismiss="dismiss" :reload-dialog="reloadDialog" :reload-source="reload"
