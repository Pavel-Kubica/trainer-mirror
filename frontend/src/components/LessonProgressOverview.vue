...
import LessonProgressOverviewHeader from '@/components/lesson/LessonProgressOverviewHeader.vue'
import LessonProgressOverviewContent from '@/components/lesson/LessonProgressOverviewContent.vue'
  ...
  lessonUserApi.lessonUserList(props.lessonId)
      .then(async (result) => {
        appState.value.navigation = [new Nav.CourseList(), new Nav.CourseDetail(result.course),
          new Nav.LessonDetail(result2, result2.week), new Nav.LessonProgressOverview(result, t)]
...
<template>
  <v-card :title="t('$vuetify.progress_overview')">
    <template #append>
    ...
    <LoadingScreen :items="lesson" :error="error">
      <template #content>
        <v-table>
          <LessonProgressOverviewHeader />
          <tbody>
            <LessonProgressOverviewContent v-for="user in lesson.users" :key="user.id" :user="user" />
          </tbody>
        </v-table>
      </template>
    </LoadingScreen>
